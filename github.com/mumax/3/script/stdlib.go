...
func (w *World) LoadStdlib() {
	...
	w.Func("randTime", randTime, "Random integer using time as the seed")
...
// var sec = time.Second(0)
var rngT = rand.New(rand.NewSource(int64(time.Now().Second())))

// script does not know int64
func intseed(seed int)      { rng.Seed(int64(seed)) }
func randInt(upper int) int { return rng.Int() % upper }
func randTime(upper int) int { return rngT.Int() % upper }
